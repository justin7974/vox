...
**How it works:** `Ctrl+`` → Record → Cloud ASR → LLM cleanup → Paste at cursor

### Features

- Global hotkey `Ctrl+`` — works in any app
...
```bash
git clone https://github.com/justin7974/voice-input.git
cd voice-input
bash build.sh
```
...
```bash
cp -r build/VoiceInput.app ~/Applications/
xattr -cr ~/Applications/VoiceInput.app   # bypass Gatekeeper
```
...
A setup window will appear on first run. Select your providers and enter API keys.
...
1. Press `Ctrl+`` to start recording (menubar icon turns red)
2. Speak naturally
...
VoiceInput uses cloud APIs. You need to bring your own keys.
...
| Problem | Solution |
|---|---|
| No audio detected | System Settings > Privacy > Microphone — allow VoiceInput |
| Text not pasting | System Settings > Privacy > Accessibility — allow VoiceInput |
| ASR errors | Check `~/.voiceinput/debug.log`, verify API key in Settings |
| macOS blocks the app | Run `xattr -cr ~/Applications/VoiceInput.app` |
...
**工作原理：** `Ctrl+`` → 录音 → 云端语音识别 → LLM 纠错 → 粘贴到光标位置

### 功能特点

- 全局快捷键 `Ctrl+`` — 在任何应用中都能用
...
```bash
cp -r build/VoiceInput.app ~/Applications/
xattr -cr ~/Applications/VoiceInput.app   # 绕过 Gatekeeper
```
...
VoiceInput 使用云端 API，你需要自己提供 Key。
...
| 问题 | 解决方案 |
|---|---|
| 检测不到音频 | 系统设置 > 隐私与安全 > 麦克风 — 允许 VoiceInput |
| 文字没有粘贴 | 系统设置 > 隐私与安全 > 辅助功能 — 允许 VoiceInput |
| 识别报错 | 查看 `~/.voiceinput/debug.log`，检查 API Key 是否正确 |
| macOS 阻止运行 | 运行 `xattr -cr ~/Applications/VoiceInput.app` |
